...
/* Extract the undo bank number from an undo log number. */
#define UndoLogNoGetBankNo(logno)				\
	((logno) >> (UndoLogNumberBits - UndoLogBankBits))

/* Extract the slot within a bank from an undo log number. */
#define UndoLogNoGetSlotNo(logno)				\
	((logno) & ((1 << UndoLogNumberBits) - 1))

/*
 * During recovery we maintain a mappind of transaction ID to undo logs
...
/* Extract the upper bits of an xid, for undo log mapping puproses. */
...
UndoLogShmemInit(void)
{
	...
	if (!IsUnderPostmaster)
	{
		...
		/*
		 * During bootstrapping, this initial state of affairs is correct:
		 * there are no undo logs.  During recovery, we'll replay an
		 * XLOG_UNDOLOG_LOGS record which will install all known undo logs
		 * into shared memory.
		 */
...
extend_undo_log(UndoLogNumber logno, UndoLogOffset capacity)
{
	...
	else
	{
		...
		fd = OpenTransientFile(path,
							   O_RDWR | O_CREAT | PG_BINARY, S_IRUSR | S_IWUSR);
		zeroes = palloc0(8192);
...
CheckPointUndoLogs(XLogRecPtr checkPointRedo)
{
	...
	snprintf(path, MAXPGPATH, "pg_undo/%016" INT64_MODIFIER "X",
			 checkPointRedo);
	fd = OpenTransientFile(path, O_RDWR | O_CREAT | PG_BINARY, S_IRUSR | S_IWUSR);
	if (fd < 0)
		ereport(ERROR,
...
StartupUndoLogs(XLogRecPtr checkPointRedo)
{
	...
	snprintf(path, MAXPGPATH, "pg_undo/%016" INT64_MODIFIER "X",
			 checkPointRedo);
	fd = OpenTransientFile(path, O_RDONLY | PG_BINARY, S_IRUSR | S_IWUSR);
	if (fd < 0)
	...
	elog(LOG, "high = %d, low = %d, num = %d", shared->high_logno, shared->low_logno, num_logs);
	for (logno = shared->low_logno; logno < shared->high_logno; ++logno)
	{
		UndoLogControl *log;
...
attach_undo_log(void)
{
	...
	if (shared->free_list != InvalidUndoLogNumber)
	{
		...
		elog(WARNING, "will take %d from free list", logno);
		log = get_undo_log_by_number(logno);
		if (log == NULL)
...
/*
 * replay the creation of a an undo log
 */
static void
